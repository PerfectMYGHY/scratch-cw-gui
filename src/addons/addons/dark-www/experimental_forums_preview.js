        ...
        if (record.type === "childList") {
          for (let node of record.addedNodes) {
            ...
            if (node.tagName === "LINK" && node.href.endsWith("djangobb_forum/css/pygments.css")) {
              preview.contentDocument.head.appendChild(createStyle(addon.self.getResource("/experimental_scratchr2.css"))) /* rewritten by pull.js */;
              preview.contentDocument.head.appendChild(
                createStyle(addon.self.getResource("/pygments.css") /* rewritten by pull.js */, !addon.settings.get("darkForumCode"))
              );
            }
          }
        }
