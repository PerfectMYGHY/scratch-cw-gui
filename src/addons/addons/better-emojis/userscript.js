export default async function ({ addon, console, msg }) {
  ...
  addon.self.addEventListener("reenabled", () => {
    ...
    for (const emoji of emojis) {
      if (EMOJIS.includes(emoji.dataset.saEmoji)) {
        // check if emoji is supported by this addon
        emoji.src = addon.self.getResource("/images/" + emoji.dataset.saEmoji + ".svg") /* rewritten by pull.js */;
      }
    }
  });
  while (true) {
    ...
    if (match !== null) {
      ...
      if (EMOJIS.includes(emojiId)) {
        // check if emoji is supported by this addon
        if (!addon.self.disabled) emoji.src = addon.self.getResource("/images/" + emojiId + ".svg") /* rewritten by pull.js */;
