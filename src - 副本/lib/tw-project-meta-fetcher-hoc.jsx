...
export const fetchProjectMeta = async (projectId, projectHost) => {
    const urls = [
        `${process.env.PROJECT_HOST || "http://192.168.8.104:8006"}/projects/${projectId}`,
        `${process.env.PROJECT_HOST || "http://192.168.8.104:8006"}/projects/${projectId}`
    ];
    let firstError;
            ...
            if (this.props.reduxProjectId !== prevProps.reduxProjectId) {
                ...
                if (projectId === '0') {
                    ...
                    fetchProjectMeta(projectId).then(data => {
                        ...
                        const authorThumbnail = data.author.image;
